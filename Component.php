...
class Component extends \yii\base\Component
{
    ...
    {
        $param = Yii::$app->request->get('language-picker-language') ?? Yii::$app->request->get('language');
        
        if ($param) {
            if ($this->_isValidLanguage($param)) {
                // ✅ Guarda en cookie Y en sesión
                Yii::$app->language = $param;
                $this->saveLanguageIntoCookie($param);
                Yii::$app->session->set('language', $param);
        ...
        // 🔄 Leer de cookie o sesión
        $cookieLang = Yii::$app->request->cookies->getValue($this->cookieName);
        $sessionLang = Yii::$app->session->get('language');
        
        if ($this->_isValidLanguage($sessionLang)) {
            Yii::$app->language = $sessionLang;
            return;
        } elseif ($this->_isValidLanguage($cookieLang)) {
            Yii::$app->language = $cookieLang;
            return;
        }
        
        // 🌍 Detectar por navegador si no hay cookie ni sesión
        $this->detectLanguage();
    }
    ...
    public function saveLanguageIntoCookie($language)
    {
        ...
        // Solo agregar SameSite=None y Secure si estamos en embed-apply
        if (
            Yii::$app->controller &&
            Yii::$app->controller->id === 'application' &&
            Yii::$app->controller->action &&
            Yii::$app->controller->action->id === 'embed-apply'
            ) {
                $options['secure'] = true;
